...
struct Inner<T> {
    join_set: JoinSet<T>,
    /// If the underlying [`JoinSet`] is empty, a waker can be registered such
    /// that the underlying [`JoinSet`] will be polled once a new task is
    /// spawned.
    waker: Option<Waker>,
}
...
/// Used to receive task results. Must be polled in order to clear completed
/// tasks from the underlying JoinSet.
