...
impl Setup {
    ...
    fn new(
        ...
        Ok(Self {
            ...
            accumulator,
            tip_hash,
            memory_pool: crate::state::MemoryPool::with_capacity_estimate(
                50000,
            ), // Estimate based on 320MB blocks
        })
    }
}
...
fn connect_blocks(
    ...
    while blocks_processed < n_blocks {
        let batch_size =
            batcher.calculate_optimal_batch_size(n_blocks - blocks_processed);
        ...
        for block in &blocks {
            setup.state.apply_block_with_pool(
                &mut rwtxn,
                &block.header,
                &block.body,
                &mut setup.memory_pool,
            )?;
        }
        rwtxn.commit()?;
...
impl AdaptiveBatcher {
    ...
    fn calculate_optimal_batch_size(&mut self, blocks_remaining: u32) -> u32 {
        ...
        let remaining_aware_batch =
            std::cmp::min(blocks_remaining, memory_based_batch);
    ...
    fn record_performance(&mut self, batch_size: u32, duration: Duration) {
        ...
        if blocks_per_second > 0.0 {
            ...
            let alpha = 0.1;
            let _estimated_block_time =
                1.0 / blocks_per_second * batch_size as f64;
            let estimated_size =
                self.target_memory_mb as f64 / batch_size as f64;
            self.avg_block_size_mb =
                alpha * estimated_size + (1.0 - alpha) * self.avg_block_size_mb;
        }
    }
}
