...
pub fn connect_prevalidated(
    ...
    let mut utxo_delete_keys: Vec<OutPointKey> = utxo_deletes
        .par_iter()
        .map(OutPointKey::from_outpoint)
        .collect();

    let mut stxo_put_data: Vec<(OutPointKey, Vec<u8>)> = stxo_puts
        .par_iter()
        .map(|(op, spent)| {
            let key = OutPointKey::from_outpoint(op);
            let serialized =
                borsh::to_vec(spent).expect("SpentOutput serialization failed");
            (key, serialized)
        })
        .collect();

    let mut utxo_put_data: Vec<(OutPointKey, Vec<u8>)> = utxo_puts
        .par_iter()
        .map(|(op, output)| {
            let key = OutPointKey::from_outpoint(op);
            let serialized =
                borsh::to_vec(output).expect("Output serialization failed");
            (key, serialized)
        })
        .collect();
    ...
    for (key, serialized_value) in &stxo_put_data {
        // Deserialize for put operation
        let spent_output: SpentOutput = borsh::from_slice(serialized_value)
            .expect("SpentOutput deserialization failed");
        state.stxos.put(rwtxn, key, &spent_output)?;
    }

    for (key, serialized_value) in &utxo_put_data {
        // Deserialize for put operation
        let output: Output = borsh::from_slice(serialized_value)
