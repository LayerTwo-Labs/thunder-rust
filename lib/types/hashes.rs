    ...
    T: BorshSerialize + Sync,
{
    ...
    let chunk_size = std::cmp::max(
        4,
        data_batch.len() / rayon::current_num_threads().max(1),
    );
    ...
    data_batch
        ...
        .flat_map(|chunk| {
            ...
            chunk
                .iter()
                .map(|item| {
                    let data_serialized = borsh::to_vec(item).expect(
                        "failed to serialize with borsh to compute a hash",
                    );
                    blake3::hash(&data_serialized).into()
                })
                .collect::<Vec<_>>()
...
pub fn hash_preserialized_batch(serialized_data: &[Vec<u8>]) -> Vec<Hash> {
    ...
    if serialized_data.len() < 4 {
        return serialized_data
            .iter()
            .map(|data| blake3::hash(data).into())
            .collect();
    }
